...
os.chdir('/home/pi/tmu-v1-conservator/')
...
class HysteresisController:
    def __init__(self, threshold_on, threshold_off):
        self.threshold_on = threshold_on
        self.threshold_off = threshold_off
        self.state = False 

    def update(self, value):
        if not self.state and value >= self.threshold_on:
            self.state = True 
...
def dataHandler(getTemp, getOil, getElect1, getElect2, getElect3, getHarmV, getHarmA, currentResult, CTratio, PTratio):
    ...
    try:
        for i in range(0, 3):
            currentResult[(i*9)+12] = (getElect2.registers[i])/10
...
def Restart():
    ...
    os.execv(sys.executable, [sys.executable] + ['/home/pi/tmu-v1-conservator/IoT_Trafo_Project.py'])
...
def mainLoop(thread_name, interval):
    ...
    WTIcontroller = HysteresisController(threshold_on=80, threshold_off=70)
        ...
        while progStat:
            ...
            if healthResult == True or trialNum >= trialNumLimit-1:
                ...
                for i in range(0, 3):
                    ...
                    stateWTI[i] = WTIcontroller.update(windTemp[i])

                #print(stateWTI)
                if all(x == False for x in stateWTI):
                    #print("Kipas Mati")
                    client.write_register(502, 0, slave=1)
                    coolingMode = "ONAN"
                else:
                    #print("Kipas Nyala")
                    client.write_register(502, 1, slave=1)
                    if coolingModel == "ONAF" :
                        coolingMode = "ONAF"
